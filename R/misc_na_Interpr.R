...
#' 
#' @param df Dataset containing variable on which NA hnandling is to be 
#'   applied to
#' @param var Character value of variable in `df` on which NA handling is to be
#'   applied to
#'   
#' @return A data.frame with same number of rows as `df` and with additional
#'   columns for imputed variables
#'   
#' @import dplyr
#' 
#' @export
...
na_interp <- function(df, var){
  ...
  if(sum(!is.na(df[,var])) > 1){
    out <- df %>%
      mutate(imputed_value = is.na(get(var))) %>%
      mutate(!!paste0(var, "_imputed") := na_interpolation(get(var)))
  }
