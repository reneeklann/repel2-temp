#' Get sample of NAPPO reports and preprocess for openai extraction
#'
#' This function creates a vector of free text reports from NAPPO
#'
...
#' 
#' @export
get_nappo_reports_sample <- function(crop_data_manual, crop_report_index) {
  
  crop_data_manual <- read.csv(crop_data_manual)
  
  nappo_reports <- crop_report_index |>
    dplyr::filter(source == "NAPPO") |>
    dplyr::filter(url %in% crop_data_manual$url) |>
    dplyr::mutate(specific_context = text) |>
    dplyr::mutate(specific_context = stringr::str_remove_all(string = specific_context, 
                                                             pattern = "Posted\\sDate\\:\\s.*"))
  
  nappo_reports
}


#' Get NAPPO reports and preprocess for openai extraction
#'
#' This function creates a vector of free text reports from NAPPO
#'
#' @return vector of free text reports
...
#' 
#' @export
get_nappo_reports <- function(crop_data_manual, crop_report_index) {
...
get_function_call_params_nappo <- function(nappo_report) {
  
  nappo_countries <- c("Country: Canada", "Country: Mexico", "Country: United States")
  nappo_countries <- stringr::str_flatten(nappo_countries, "|")
  country <- nappo_report |>
    stringr::str_extract(pattern = nappo_countries) |>
  ...
  function_call_df <- tibble::tribble(~parameter_name, ~description, ~type, ~enum,
                                      ...
                                      # "year", "The year the disease or pest was detected", c("string", "null"), NA, #  If the year is not mentioned, return 'NA'.
                                      "year", "The year the outbreak event occurred", c("string", "null"), NA,
                                      # "month", "The month the disease or pest was detected. If the month is not mentioned, return 'NA'.", c("string", "null"), c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December", "NA"),
                                      "month", "The month the outbreak event occurred. If the month is not mentioned, return 'NA'.", c("string", "null"), c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December", "NA"),
                                      "host", "The species name of the plant affected by the disease or pest. If the species name is not mentioned, return the common name. If neither the species name nor the common name is mentioned, return 'NA'. Do not extract terms such as 'deciduous trees', 'shrubs', or 'fruit fly'. Do not extract a place name.", c("string", "null"), NA,
                                      "presence", paste0("Is the disease or pest present or absent from ", country, "? This information often follows the phrase 'Under IPPC Standards'."), c("string", "null"), c("present", "absent", "NA")
  ...
  function_call_df <- function_call_df |>
    dplyr::rowwise() |> dplyr::mutate(parameter = get_function_call_parameter_nappo(parameter_name, description, type, enum))
...
get_function_call_parameter_nappo <- function(parameter_name,
                                              description,
                                              type = c("string", "null"),
                                              enum = NULL) {
...
get_openai_function_call_nappo <- function(nappo_report,
  ...
  function_call_df <- get_function_call_params_nappo(nappo_report)
...
extract_data_nappo <- function(nappo_reports, 
                               # model = "gpt-3.5-turbo", 
  ...
  if(pipeline == "full"){
    file_name <- paste0("nappo_", janitor::make_clean_names(unique(nappo_reports$preferred_name)), ".gz.parquet")
  }
  if(pipeline == "assessment"){
    file_name <- "nappo_assessment.gz.parquet"
  }
  ...
  urls <- nappo_reports$url
  nappo_reports <- nappo_reports$specific_context
  
  openai_chat <- purrr::map2_dfr(nappo_reports, urls, function(nappo_report, url){
    
    # nappo_report <- nappo_reports[1]
    
    specific_context <- nappo_report
    ...
    function_call = get_openai_function_call_nappo(nappo_report)
