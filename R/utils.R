...
all_targets <- function(env = parent.env(environment()), type = "tar_target", add_list_names = TRUE) {
  ...
  # Get the names everything in the environment (e.g. sourced in the _targets.R file)
  objs <- ls(env)
  ...
  for(o in objs) {
    ...
    if (rfn(obj)) { # For targets and lists of targets
      ...
      # If the object is a list of targets, add a vector of the target names to the environment
